...
/*
	...
	log("obj = 0x%x\n", obj)
*/
    //log("provo a caricare un file .dex\n");
    //provo a caricare un dex
    int val = dexstuff_loaddex(&d, "/data/local/tmp/file.dex");
    void *clazz = dexstuff_defineclass(&d, "MainActivity", val);
    log("caricato dex: clazz = 0x%x\n", clazz)

    /*
    jvalue args[1];
    jclass mia = (*env)->FindClass(env, "com/example/prova/MainActivity");
    jmethodID miometodo = (*env)->GetStaticMethodID(env, mia, "pippo","()I");
    if( miometodo == 0)
        log("errore get staticmethodid!!");
    jint result = (*env)->CallStaticIntMethod(env,mia, miometodo, args);
    log("risultato: %d\n",result);
    */
